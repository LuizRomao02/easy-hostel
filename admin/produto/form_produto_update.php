...
<body id="page-top">
	...
	$sth = $pdo->prepare("select *from pro_produto where pro_id = :id");
	...
	<div id="content-wrapper">
		...
		<div class="container-fluid">
			...
			<ol class="breadcrumb">
				<li class="breadcrumb-item active">Atualizar Informações do Produto</li>
			</ol>

			<form class="card m-5" onsubmit="insert(event); return false" method="post">
				<div class="card-header"><i class="far fa-edit mr-2"></i>Atualizar Informações do Produto</div>
				<div class="form-row m-4">
					<div class="col-md-12 mb-4">
						<input type="hidden" name="id" value="<?= $pro_id; ?>" />
						<label for="Nome">Alterar Nome</label>
						<input type="text" class="form-control" name="nome" placeholder="Nome" value="<?= $pro_nome; ?>" required>
					</div>
					<div class="col-md-6 mb-4">
						<label for="Marca">Alterar Marca</label>
						<input type="text" class="form-control" name="marca" placeholder="Marca" value="<?= $pro_marca; ?>" required>
					</div>
					<div class="col-md-6 mb-4">
						<label for="Preco">Alterar Preço</label>
						<input type="text" class="form-control" name="preco" id="preco" placeholder="Preço" value="<?= $pro_preco; ?>" required>
					</div>
	...
	<script>
		...
		async function insert(e) {
			...
			const data = await (await fetch('update_produto.php', {
				method: 'POST',
				body: formData
			})).json()
