...
<body id="page-top">
	...
	$sth = $pdo->prepare("select *from for_formapag where for_id = :id");
	...
	<div id="content-wrapper">
		...
		<div class="container-fluid">
			...
			<ol class="breadcrumb">
				<li class="breadcrumb-item active">Atualizar Informações de Tipo de Pagamento</li>
			</ol>

			<form class="card m-5" onsubmit="insert(event); return false" method="post">
				<div class="card-header"><i class="far fa-edit mr-2"></i>Atualizar Informações de Tipo de Pagamento</div>
				<div class="form-row m-4">
					<div class="col-md-12 mb-4">
						<input type="hidden" name="id" value="<?= $for_id; ?>" />
						<label for="Tipo">Alterar Tipo de Pagamento</label>
						<input type="text" class="form-control" name="tipo" placeholder="Tipo de Pagamento" value="<?= $for_tipo; ?>" required>
					</div>
					<div class="col-md-12 mb-4">
						<label for="Descricao">Alterar Descrição</label>
						<input type="text" class="form-control" name="descricao" placeholder="Descrição" value="<?= $for_descricao; ?>" required>
					</div>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<p>A Forma de Pagamento foi atualizada com Sucesso!</p>
					</div>
	...
	<script>
		...
		async function insert(e) {
			...
			const data = await (await fetch('update_pagamento.php', {
				method: 'POST',
				body: formData
			})).json()
